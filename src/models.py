...
class BillSection(BaseModel):
    ...
    key_points: List[str]
    defined_terms: Dict[str, str] = Field(default_factory=dict, description="Important terms defined in this section")
    impacts: List[LegislativeImpact]
    referenced_sections: List[str] = Field(default_factory=list, description="Other sections referenced by this section")
    funding_amounts: Optional[Dict[str, Any]] = Field(None, description="Funding allocations (can be numbers or descriptive text)")
